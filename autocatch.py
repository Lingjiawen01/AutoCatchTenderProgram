...
class SiteParser:
    ...
    def seb_energy(block, lines, idx):
        ...
        # 逐行向后找真正标题，遇到 Eligible Tenderers 或日期立即截断
        title = "Check Portal"
        for j in range(idx + 1, min(len(lines), idx + 10)):
            line = lines[j].strip()
            upper_l = line.upper()
            
            # 遇到 Eligible Tenderers、Publish Date、Close Date 等标签立即止步
            if any(k in upper_l for k in ["ELIGIBLE TENDERERS", "PUBLISH DATE", "CLOSING DATE", "CLOSE DATE"]):
                break
                
            # 过滤不需要的杂行与独立 Doc ID 行
            if len(line) > 8 and not line.startswith("Doc") and not line.isdigit():
                # 如果同一行里夹带了 Eligible Tenderers，切掉后半段
                clean_t = re.split(r'Eligible\s+Tenderers', line, flags=re.I)[0].strip()
                if len(clean_t) > 5:
                    title = clean_t[:150]  # 限制Telegram消息长度
                    break
